...
#convolution filters parameters
...
num_first_fully=512 # = Number of neurons in first fully connected
num_second_fully=256 # = Number of neurons in second fully connected
...
##############################################################
#Function that computes convolutions for input#
##############################################################
def convolution_computation(convolutions, input, validation_input):
    conv_input = input
    valid_conv_input = validation_input
    for convolution, bias in convolutions:    
        conv1=conv2d(conv_input,convolution)
        conv1_valid=conv2d(valid_conv_input,convolution)
        out=tf.nn.bias_add(conv1,bias)
        out_valid=tf.nn.bias_add(conv1_valid,bias)
        out=tf.nn.relu(out)
        out_valid=tf.nn.relu(out_valid)
    
        conv_input = out
        valid_conv_input = out_valid     
    
    return (out,out_valid)
    
def create(train_data, validation_data, keep_probability=1.0, num_classes=None):
    #####################
    #We define the model#
    #####################
    with tf.name_scope('Conv_Block_1'):
        ...
        #definition of the weights for the first block
        convolutions = create_convolutions(3, num_first_convolutions)
        #end of definition
        
        #convolution computation
        (out,out_valid)=convolution_computation(convolutions, train_data, validation_data)
        #end convolution computation
    ...
    with tf.name_scope('Conv_Block_2'):
        ...
        #definition of the weights for the second block
        convolutions = create_convolutions(num_first_convolutions, num_second_convolutions)
        #end of definition

        #convolution computation
        (out,out_valid)=convolution_computation(convolutions, pool1,pool1_valid)
        #end convolution computation
    ...
    with tf.name_scope('Conv_Block_3'):
        #definition of the weights for the third block
        convolutions=create_convolutions(num_second_convolutions, num_third_convolutions)
        #end of definition
        
        #convolution computation
        (out,out_valid)=convolution_computation(convolutions, pool2,pool2_valid)
        #end convolution computation
    ...
    with tf.name_scope('Conv_Block_4'):
        #definition of the weights for the third block
        convolutions=create_convolutions(num_third_convolutions, num_fourth_convolutions)
        #end of definition
        
        #convolution computation
        (out,out_valid)=convolution_computation(convolutions, pool3,pool3_valid)
        #end convolution computation
        #Note that fourht convolution block has no pooling layer
    ...
    with tf.name_scope('first_fully_connected_layer'):
        ...
        #we get the product of the shape of the last pool to flatten it, here 40960
        '''CONCAT'''
        pool4=tf.concat([out], 2)
        pool4_valid=tf.concat([out_valid], 2)
        '''END OF CONCAT'''
        shape = int(np.prod(pool4.get_shape()[1:]))   
        first_fc=weight_variables([shape,num_first_fully],'weights')
        first_fc_bias=bias_variables([num_first_fully],'bias')
    ...
    return (classification, classification_valid)
