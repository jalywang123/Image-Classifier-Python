...
with open("train-05.csv","rb") as csvfile : 
	...
	for row in linereader:
		...
		if(set=="train"):
			...
			filename=element[0].split(".")
			first=filename[0]+".png"
			if(len(element)==1):
				second=filename[0]+".png"
			else: 
				second=filename[0]+".png"
			...
			if(resolution=="1628_930"):
				...
				if(presence_1 and presence_2):
					...
					if(label=="0" and train_ok==num_train and valid_ok<num_valid):
						save_set="valid"
						valid_ok=valid_ok+1
						SavePath_1="%s/clean_data/%s/%s/%08d_1%s" % (dst_dir,save_set, status, valid_ok, format)
						SavePath_2="%s/clean_data/%s/%s/%08d_2%s" % (dst_dir,save_set, status, valid_ok, format)
					...
					if(label=="1"):
						if(status=="PLUSIEURS_VEHICULES" and train_plus==num_train and valid_plus<num_valid):
							...
							SavePath_1="%s/clean_data/%s/%s/%08d_1%s" % (dst_dir,save_set, status, valid_plus, format)
							SavePath_2="%s/clean_data/%s/%s/%08d_2%s" % (dst_dir,save_set, status, valid_plus, format)
							cv2.imwrite(SavePath_1,crop_1)
							cv2.imwrite(SavePath_2,crop_2)
					...
					if(label=="0" and train_ok<num_train):
						train_ok=train_ok+1
						save_set="train"
						SavePath_1="%s/clean_data/%s/%s/%08d_1%s" % (dst_dir,save_set, status, train_ok, format)
						SavePath_2="%s/clean_data/%s/%s/%08d_2%s" % (dst_dir,save_set, status, train_ok, format)
					...
					if(label=="1"): 						
						if(status=="PLUSIEURS_VEHICULES" and train_plus<num_train):
							...
							save_set="train"
							SavePath_1="%s/clean_data/%s/%s/%08d_1%s" % (dst_dir,save_set, status, train_plus, format)
							SavePath_2="%s/clean_data/%s/%s/%08d_2%s" % (dst_dir,save_set, status, train_plus, format)
			...
			if(resolution=="2240_1488"):
				...
				if(presence_1 or presence_2):
					...
					if(label=="0" and train_ok==num_train and valid_ok<num_valid):
						save_set="valid"
						valid_ok=valid_ok+1
						SavePath_2="%s/clean_data/%s/%s/%08d%s" % (dst_dir,save_set, status, valid_ok, format)
						cv2.imwrite(SavePath_2,crop_2)
					...
					if(label=="1"):						
						if(status=="PLUSIEURS_VEHICULES" and train_plus==num_train and valid_plus<num_valid):
							save_set="valid"
							valid_plus=valid_plus+1
							SavePath_2="%s/clean_data/%s/%s/%08d%s" % (dst_dir,save_set, status, valid_plus, format)
							cv2.imwrite(SavePath_2,crop_2)
			
					if(label=="0" and train_ok<num_train):
						train_ok=train_ok+1
						save_set="train"
						SavePath_2="%s/clean_data/%s/%s/%08d%s" % (dst_dir,save_set, status, train_ok, format)
						cv2.imwrite(SavePath_2,crop_2)
					if(label=="1"):				
						if(status=="PLUSIEURS_VEHICULES" and train_plus<num_train):
							train_plus=train_plus+1
							save_set="train"
							SavePath_2="%s/clean_data/%s/%s/%08d%s" % (dst_dir,save_set, status, train_plus, format)
							cv2.imwrite(SavePath_2,crop_2)
				else:
					print("the files do not exist")
